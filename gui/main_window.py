...
class MainWindow(QMainWindow):
    ...
    def on_analysis_finished(self, result):
        ...
        self.act_check.setEnabled(True) # Bağlantı kontrolü artık yapılabilir
        
        self.log(f"Analiz Bitti. {len(result.structural_groups)} hat bulundu.")
        self.log("Şimdi 'Kutu Çiz' moduna geçip hattın uçlarına kutu çizin.")

    def run_connection_check(self):
        # 1. Çizilen kutuları al
        boxes = self.viewer.get_drawn_components()
        if not boxes:
            self.log("UYARI: Hiç kutu çizmediniz! Önce 'Kutu Çiz' butonuna basın.")
            return
        ...
        for net_id, box_ids in connections.items():
            if len(box_ids) >= 2:
                ...
                self.log(f"✅ BAĞLI: {connected_str}, {net_id} hattı üzerinden birbirine bağlı!")
                found_any = True
            elif len(box_ids) == 1:
                self.log(f"ℹ️ {box_ids[0]}, {net_id} hattına temas ediyor ama hattın diğer ucu boş.")
        
        if not found_any:
            self.log("❌ İki kutu arasında bir bağlantı bulunamadı.")
            self.log("İpucu: Kutuların hatların (çizgilerin) üzerine geldiğinden emin olun.")
    ...
    def on_analysis_finished(self, result):
        ...
        self.act_ocr_test.setEnabled(True) # <--- BU BUTONU AKTİF ET
