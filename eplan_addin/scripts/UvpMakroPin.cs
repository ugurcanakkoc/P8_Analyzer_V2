// =====================================================================================
// UvpMakroPin.cs — paketteki ürün kodlarının makrolarını ve makro uçlarını dosyaya yazar.
//
// Kullanım: EPLAN > Yardımcı programlar > Script'ler > Çalıştır… > bu dosya
//   1) Paket dosyasını seç (output\exchange\*_tum_sayfalar.json)
//   2) Her ürün kodu parça veritabanında aranır, makrosu geçici projede açılır,
//      fonksiyon uçları okunur, proje kapatılır. AÇIK PROJENE DOKUNULMAZ.
//
// Çıktı: output\exchange\makro_pinleri.json
// =====================================================================================
using System;
using System.IO;
...
public class UvpMakroPin
{
    ...
    const string Baslik = "UVP · makro uçları";
    ...
    public void Basla()
    {
        ...
        string cikti = Path.Combine(Kok, @"output\exchange");
        ...
        {
            dialog.Title = "Paket (JSON) seç — ürün kodları buradan okunur";
            dialog.Filter = "Paket (*.json)|*.json";
            dialog.InitialDirectory = cikti;
        ...
        bool ok = cli.Execute("UvpPdfToP8Macros /PACKAGE:\"" + paket + "\" /OUT:\"" + cikti +
                              "\" /TEMPLATE:\"" + Sablon + "\"");
        string dosya = Path.Combine(cikti, "makro_pinleri.json");
